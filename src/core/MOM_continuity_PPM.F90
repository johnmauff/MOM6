...
implicit none ; private
  ...
  interface
    ...
    subroutine turbotmp_zonal_flux_thickness_bridge(bx, u, h, h_W, h_E, h_u, dt, dy_Cu, IareaT, &
      ...
      type(RealArray_C), intent(in)        :: h              !< Layer thickness used to calculate
      ...
      type(RealArray_C), intent(in)        :: h_E            !< East edge thickness in the
                                                              !! reconstruction [H ~> m or kg m-2].
      type(RealArray_C), intent(inout)     :: h_u            !< Effective thickness at zonal
                                                              !! faces [H ~> m or kg m-2].
      real(c_double),    intent(in), value :: dt             !< Time increment [T ~> s].
      type(RealArray_C), intent(in)        :: dy_Cu          !< The grid cell's unblocked lengths
                                                              !! of the u/v-faces [L ~> m].
      ...
      type(RealArray_C), intent(in)        :: visc_rem_u     !< Fraction of momentum remaining
                                                              !! after a time-step of viscosity
                                                              !! [nondim]
      type(c_ptr),       intent(in), value :: obc            !< Pointer to OBC structure
  ...
  interface
    ...
    subroutine turbotmp_meridional_flux_thickness_bridge(bx, v, h, h_S, h_N, h_v, dt, dx_Cv, &
      ...
      type(Box_C),       intent(in)        :: bx             !< Index space over which to iterate
      type(RealArray_C), intent(in)        :: v              !< Meridional velocity
                                                              !! [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: h              !< Layer thickness used to calculate
                                                              !! fluxes [H ~> m or kg m-2].
      ...
      type(RealArray_C), intent(in)        :: h_N            !< North edge thickness in the
                                                              !! reconstruction [H ~> m or kg m-2].
      type(RealArray_C), intent(inout)     :: h_v            !< Effective thickness at meridional
                                                              !! faces [H ~> m or kg m-2].
      real(c_double),    intent(in), value :: dt             !< Time increment [T ~> s].
      type(RealArray_C), intent(in)        :: dx_Cv          !< The grid cell's unblocked lengths
                                                              !! of the u/v-faces [L ~> m].
      type(RealArray_C), intent(in)        :: IareaT         !< The grid cell's 1/areaT
                                                              !! [L-2 ~> m-2].
  ...
  interface
    ...
    subroutine turbotmp_continuity_zonal_convergence_bridge(bx, h, uh, dt, IareaT, hin, &
                                           hmin) bind(C)
      ...
      type(Box_C),       intent(in)        :: bx           !< Index space over which to iterate
      type(RealArray_C), intent(inout)     :: h             !< Final layer thickness
                                                             !! [H ~> m or kg m-2]
      ...
      type(RealArray_C), intent(in)        :: IareaT        !< The grid cell's 1/areaT
                                                             !! [L-2 ~> m-2].
      type(RealArray_C), intent(in)        :: hin           !< Initial layer thickness
                                                             !! [H ~> m or kg m-2]. Unassociated
                                                             !! (rank 0) if hin is absent
                                                             !! Fortran-side.
      real(c_double),    intent(in), value :: hmin          !< The minimum layer thickness
                                                             !! [H ~> m or kg m-2]
  ...
  interface
    ...
    subroutine turbotmp_continuity_meridional_convergence_bridge(bx, h, vh, dt, IareaT, hin, &
                                           hmin) bind(C)
      use iso_c_binding, only : c_double
      use array_mod, only : RealArray_c
      use box_mod,   only : Box_c
  ...
  interface
    !> Bridge for the zonal_mass_flux subroutine
    subroutine turbotmp_zonal_mass_flux_bridge(bx, u, h_in, h_W, h_E, uh, dt, obc, &
      ...
      type(Box_C),       intent(in)        :: bx     !< Index space over which to iterate
      type(RealArray_C), intent(in)        :: u      !< Zonal velocity [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: h_in   !< Layer thickness used to calculate fluxes
                                                      !! [H ~> m or kg m-2].
      type(RealArray_C), intent(in)        :: h_W    !< Western edge thicknesses
                                                      !! [H ~> m or kg m-2].
      type(RealArray_C), intent(in)        :: h_E    !< Eastern edge thicknesses
                                                      !! [H ~> m or kg m-2].
      type(RealArray_C), intent(inout)     :: uh     !< Volume flux through zonal faces = u*h*dy
                                                      !! [H L2 T-1 ~> m3 s-1 or kg s-1].
      real(c_double),    intent(in), value :: dt     !< Time increment [T ~> s].
      type(c_ptr),       intent(in), value :: obc    !< Pointer to OBC structure
      type(RealArray_C), intent(in)        :: por_face_areaU !< fractional open area of U-faces
                                                             !! [nondim]
      type(RealArray_C), intent(in)        :: uhbt   !< The summed volume flux through zonal
                                                      !! faces [H L2 T-1 ~> m3 s-1 or kg s-1].
                                                      !! Unassociated (rank 0) if absent
                                                      !! Fortran-side.
      type(RealArray_C), intent(in)        :: visc_rem_u !< The fraction of zonal momentum
                                                         !! originally in a layer that remains
                                                         !! after a time-step of viscosity
                                                         !! [nondim]. Unassociated (rank 0) if
                                                         !! absent Fortran-side.
      type(RealArray_C), intent(inout)     :: u_cor  !< The zonal velocities (u with a
                                                      !! barotropic correction) that give uhbt
                                                      !! as the depth-integrated transport
                                                      !! [L T-1 ~> m s-1]
      type(c_ptr),       intent(in), value :: bt_cont !< Pointer to a BT_cont_type structure
                                                       !! with elements that describe the
                                                       !! effective open face areas as a
                                                       !! function of barotropic flow, or a
                                                       !! null pointer if absent Fortran-side.
      type(RealArray_C), intent(inout)     :: du_cor !< The zonal velocity increments from u
      ...
      type(RealArray_C), intent(in)        :: dy_Cu  !< The grid cell's unblocked lengths of
                                                      !! the u/v-faces of the h-cell [L ~> m].
      ...
      type(RealArray_C), intent(in)        :: areaT  !< The grid cell's area [L2 ~> m2].
      type(RealArray_C), intent(in)        :: dxT    !< The dx spacing at h points [L ~> m].
      type(RealArray_C), intent(in)        :: mask2dCu !< 0 for boundary points and 1 for ocean
  ...
  interface
    !> Bridge for the meridional_mass_flux subroutine
    subroutine turbotmp_meridional_mass_flux_bridge(bx, v, h_in, h_S, h_N, vh, dt, obc, &
      ...
      type(RealArray_C), intent(in)        :: h_N    !< North edge thickness in the
                                                      !! reconstruction [H ~> m or kg m-2].
      type(RealArray_C), intent(inout)     :: vh     !< Volume flux through meridional faces =
      ...
      type(RealArray_C), intent(in)        :: vhbt   !< The summed volume flux through
                                                      ...
                                                      !! Unassociated (rank 0) if absent
                                                      !! Fortran-side.
      type(RealArray_C), intent(in)        :: visc_rem_v !< The fraction of meridional momentum
                                                         !! originally in a layer that remains
                                                         !! after a time-step of viscosity
                                                         !! [nondim]. Unassociated (rank 0) if
                                                         !! absent Fortran-side.
      type(RealArray_C), intent(inout)     :: v_cor  !< The meridional velocities (v with a
                                                      !! barotropic correction) that give vhbt
                                                      !! as the depth-integrated transport
                                                      !! [L T-1 ~> m s-1].
      type(c_ptr),       intent(in), value :: bt_cont !< Pointer to a BT_cont_type structure
                                                       !! with elements that describe the
                                                       ...
                                                       !! function of barotropic flow, or a
                                                       !! null pointer if absent Fortran-side.
      type(RealArray_C), intent(inout)     :: dv_cor !< The meridional velocity increments from
                                                      !! v that give vhbt as the
                                                      !! depth-integrated transports
                                                      !! [L T-1 ~> m s-1]. Unassociated (rank 0)
      ...
      type(RealArray_C), intent(in)        :: areaT  !< The grid cell's area [L2 ~> m2].
      type(RealArray_C), intent(in)        :: dyT    !< The dy spacing at h points [L ~> m].
      type(RealArray_C), intent(in)        :: mask2dCv !< 0 for boundary points and 1 for ocean
  ...
  interface
    !> Bridge for the present_uhbt_or_set_BT_cont subroutine
    subroutine turbotmp_present_uhbt_or_set_bt_cont_bridge(bx, u, h_in, h_W, h_E, uh_tot_0, &
      ...
      type(RealArray_C), intent(in)        :: h_E    !< Eastern edge thicknesses
                                                      !! [H ~> m or kg m-2].
      type(RealArray_C), intent(in)        :: uh_tot_0 !< Summed transport with no barotropic
                                                        !! correction
                                                        !! [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C), intent(in)        :: duhdu_tot_0 !< Summed partial derivative of uh
                                                           !! with u [H L ~> m2 or kg m-1].
      type(RealArray_C), intent(inout)     :: du     !< Corrective barotropic change in the
                                                      !! velocity to give uhbt [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: du_max_CFL !< Upper limit on du correction to
                                                          !! avoid CFL violations [L T-1 ~> m s-1]
      type(RealArray_C), intent(in)        :: du_min_CFL !< Lower limit on du correction to
                                                          !! avoid CFL violations [L T-1 ~> m s-1]
      type(RealArray_C), intent(in)        :: visc_rem_u !< The fraction of zonal momentum
                             ...
                             !! [nondim]. Between 0 (at the bottom) and 1 (far above the bottom).
      type(RealArray_C), intent(in)        :: visc_rem_max !< The column maximum of visc_rem
                                                            !! [nondim].
      type(RealArray_C), intent(in)        :: por_face_areaU !< fractional open area of U-faces
                                                              !! [nondim]
      type(RealArray_C), intent(in)        :: uhbt   !< The summed volume flux through zonal
                                                      !! faces [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C), intent(inout)     :: uh     !< Volume flux through zonal faces =
      ...
      type(RealArray_C), intent(inout)     :: du_cor !< The zonal velocity increments from u
                                                      !! that give uhbt as the depth-integrated
                                                      !! transports [L T-1 ~> m s-1].
      type(BT_cont_C),   intent(inout)     :: bt_cont !< A structure with elements that describe
      ...
      type(RealArray_C), intent(in)        :: dy_Cu  !< The grid cell's unblocked lengths of
                                                      !! the u/v-faces of the h-cell [L ~> m].
      real(c_double),    intent(in), value :: H_subroundoff !< A thickness that is so small
                             !! that it can be added to a thickness of Angstrom or larger without
  ...
  interface
    !> Bridge for the present_vhbt_or_set_BT_cont subroutine
    subroutine turbotmp_present_vhbt_or_set_bt_cont_bridge(bx, v, h_in, h_S, h_N, vh_tot_0, &
      ...
      type(RealArray_C), intent(in)        :: h_N    !< North edge thickness in the
                                                      !! reconstruction [H ~> m or kg m-2].
      type(RealArray_C), intent(in)        :: vh_tot_0 !< Summed transport with no barotropic
                                                        !! correction
                                                        !! [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C), intent(in)        :: dvhdv_tot_0 !< Summed partial derivative of vh
                                                           !! with v [H L ~> m2 or kg m-1].
      type(RealArray_C), intent(inout)     :: dv     !< Corrective barotropic change in the
                                                      !! velocity to give vhbt [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: dv_max_CFL !< Upper limit on dv correction to
                                                          !! avoid CFL violations [L T-1 ~> m s-1]
      type(RealArray_C), intent(in)        :: dv_min_CFL !< Lower limit on dv correction to
                                                          !! avoid CFL violations [L T-1 ~> m s-1]
      type(RealArray_C), intent(in)        :: visc_rem_v !< Both the fraction of the momentum
                             !! originally in a layer that remains after a time-step of viscosity,
                             !! and the fraction of a time-step's worth of a barotropic
                             !! acceleration that a layer experiences after viscosity is applied
                             !! [nondim]. Between 0 (at the bottom) and 1 (far above the bottom).
      ...
      type(RealArray_C), intent(inout)     :: v_cor  !< The meridional velocities (v with a
                                                      ...
                                                      !! as the depth-integrated transport
                                                      !! [L T-1 ~> m s-1].
      type(RealArray_C), intent(inout)     :: dv_cor !< The meridional velocity increments
                                                      !! from v that give vhbt as the
                                                      !! depth-integrated transports
                                                      !! [L T-1 ~> m s-1].
      type(BT_cont_C),   intent(inout)     :: bt_cont !< A structure with elements that describe
                             !! the effective open face areas as a function of barotropic flow.
  ...
  interface
    ...
    subroutine turbotmp_set_zonal_bt_cont_bridge(bx, u, h_in, h_W, h_E, bt_cont, du0, &
      ...
      type(RealArray_C),    intent(in)        :: h_E    !< East edge thickness in the
                                                         !! reconstruction [H ~> m or kg m-2].
      type(BT_cont_C),      intent(inout)     :: bt_cont !< A structure with elements that
                             !! describe the effective open face areas as a function of
                             !! barotropic flow.
      type(RealArray_C),    intent(in)        :: du0    !< The barotropic velocity increment
                                                         !! that gives 0 transport [L T-1 ~> m s-1].
      type(RealArray_C),    intent(in)        :: uh_tot_0 !< The summed transport with 0
                                                           !! adjustment
                                                           !! [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C),    intent(in)        :: duhdu_tot_0 !< The partial derivative of
                             !! du_err with du at 0 adjustment [H L ~> m2 or kg m-1].
      type(RealArray_C),    intent(in)        :: du_max_CFL !< Maximum acceptable value of du
                                                             !! [L T-1 ~> m s-1].
      ...
      type(RealArray_C),    intent(in)        :: visc_rem !< Both the fraction of the momentum
                             ...
                             !! acceleration that a layer experiences after viscosity is applied
                             !! [nondim]. Between 0 (at the bottom) and 1 (far above the bottom).
      type(RealArray_C),    intent(in)        :: visc_rem_max !< Maximum allowable visc_rem
                                                               !! [nondim].
      type(LogicalArray_C), intent(in)        :: do_I   !< A logical flag indicating which I
                                                         !! values to work on.
  ...
  interface
    ...
    subroutine turbotmp_set_merid_bt_cont_bridge(bx, v, h_in, h_S, h_N, bt_cont, dv0, &
      ...
      type(RealArray_C),    intent(in)        :: dv0    !< The barotropic velocity increment
                                                         !! that gives 0 transport [L T-1 ~> m s-1].
      type(RealArray_C),    intent(in)        :: vh_tot_0 !< The summed transport with 0
                                                           !! adjustment
                                                           !! [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C),    intent(in)        :: dvhdv_tot_0 !< The partial derivative of
                             !! du_err with dv at 0 adjustment [H L ~> m2 or kg m-1].
      type(RealArray_C),    intent(in)        :: dv_max_CFL !< Maximum acceptable value of dv
                                                             !! [L T-1 ~> m s-1].
  ...
  interface
    ...
    subroutine turbotmp_zonal_flux_adjust_bridge(bx, u, h_in, h_W, h_E, uh_tot_0, &
      ...
      type(RealArray_C),    intent(in)        :: h_E !< East edge thickness in the reconstruction
                                                      !! [H ~> m or kg m-2].
      type(RealArray_C),    intent(in)        :: uh_tot_0 !< The summed transport with 0
                             !! adjustment [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C),    intent(in)        :: duhdu_tot_0 !< The partial derivative of du_err
                             !! with du at 0 adjustment [H L ~> m2 or kg m-1].
      type(RealArray_C),    intent(inout)     :: du !< The barotropic velocity adjustment
                                                     !! [L T-1 ~> m s-1].
      type(RealArray_C),    intent(in)        :: du_max_CFL !< Maximum acceptable value of du
                                                             !! [L T-1 ~> m s-1].
      type(RealArray_C),    intent(in)        :: du_min_CFL !< Minimum acceptable value of du
      ...
      type(RealArray_C),    intent(in)        :: visc_rem !< Both the fraction of the momentum
                             ...
                             !! acceleration that a layer experiences after viscosity is applied
                             !! [nondim]. Between 0 (at the bottom) and 1 (far above the bottom).
      type(LogicalArray_C), intent(in)        :: do_I_in !< A logical flag indicating which I
                                                          !! values to work on.
      type(RealArray_C),    intent(in)        :: por_face_areaU !< fractional open area of
                                                                 !! U-faces [nondim].
      type(RealArray_C),    intent(in)        :: uhbt !< The summed volume flux through zonal
                             !! faces [H L2 T-1 ~> m3 s-1 or kg s-1]. Unassociated (rank 0) if
                             !! absent Fortran-side.
      type(RealArray_C),    intent(inout)     :: uh_3d !< Volume flux through zonal faces =
                             !! u*h*dy [H L2 T-1 ~> m3 s-1 or kg s-1]. Unassociated (rank 0) if
                             !! absent Fortran-side.
      type(c_ptr),          intent(in), value :: obc  !< Pointer to OBC structure, or a null
                                                       !! pointer if absent Fortran-side.
      type(RealArray_C),    intent(in)        :: dy_Cu !< The grid cell's unblocked lengths of
                                                        !! the u/v-faces of the h-cell [L ~> m].
      type(RealArray_C),    intent(in)        :: IareaT !< The grid cell's 1/areaT [L-2 ~> m-2].
      type(RealArray_C),    intent(in)        :: IdxT !< The grid cell's 1/dxT [L-1 ~> m-1].
  ...
  interface
    ...
    subroutine turbotmp_meridional_flux_adjust_bridge(bx, v, h_in, h_S, h_N, vh_tot_0, &
      ...
      type(RealArray_C),    intent(in)        :: vh_tot_0 !< The summed transport with 0
                             !! adjustment [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C),    intent(in)        :: dvhdv_tot_0 !< The partial derivative of dv_err
                             !! with dv at 0 adjustment [H L ~> m2 or kg m-1].
      type(RealArray_C),    intent(inout)     :: dv !< The barotropic velocity adjustment
                                                     !! [L T-1 ~> m s-1].
      type(RealArray_C),    intent(in)        :: dv_max_CFL !< Maximum acceptable value of dv
                                                             !! [L T-1 ~> m s-1].
      type(RealArray_C),    intent(in)        :: dv_min_CFL !< Minimum acceptable value of dv
      ...
      type(RealArray_C),    intent(in)        :: visc_rem !< Both the fraction of the momentum
                             ...
                             !! acceleration that a layer experiences after viscosity is applied
                             !! [nondim]. Between 0 (at the bottom) and 1 (far above the bottom).
      type(LogicalArray_C), intent(in)        :: do_I_in !< A flag indicating which I values to
                                                          !! work on.
      type(RealArray_C),    intent(in)        :: por_face_areaV !< fractional open area of
                                                                 !! V-faces [nondim]
      type(RealArray_C),    intent(in)        :: dx_Cv !< The grid cell's unblocked lengths of
      ...
      type(RealArray_C),    intent(in)        :: IdyT !< The grid cell's 1/dyT [L-1 ~> m-1].
      type(RealArray_C),    intent(in)        :: vhbt !< The summed volume flux through
                             !! meridional faces [H L2 T-1 ~> m3 s-1 or kg s-1]. Unassociated
                             !! (rank 0) if absent Fortran-side.
      type(RealArray_C),    intent(inout)     :: vh_3d !< Volume flux through meridional faces =
                             !! v*h*dx [H L2 T-1 ~> m3 s-1 or kg s-1]. Unassociated (rank 0) if
                             !! absent Fortran-side.
      type(c_ptr),          intent(in), value :: obc  !< Pointer to OBC structure, or a null
                                                       !! pointer if absent Fortran-side.
  ...
  interface
    !> Bridge for the continuity_PPM subroutine
    subroutine turbotmp_continuity_ppm_bridge(bx, u, v, hin, h, uh, vh, dt, &
                                       mask2dT, IareaT, dy_Cu, IdxT, dxCu, areaT, dxT, &
      ...
      type(Box_C),       intent(in)        :: bx  !< The continuity solver's base (unwidened)
                                                   !! iteration box; widened variants derive from
                                                   !! it.
      type(RealArray_C), intent(in)        :: u   !< Zonal velocity [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: v   !< Meridional velocity [L T-1 ~> m s-1].
      ...
      type(RealArray_C), intent(in)        :: por_face_areaV !< fractional open area of V-faces
                                                              !! [nondim]
      type(c_ptr),       intent(in), value :: obc !< Pointer to OBC structure, or a null pointer
                                                   !! if absent Fortran-side.
      integer(c_int),    intent(in), value :: first_direction !< An integer that indicates which
      ...
      real(c_double),    intent(in), value :: H_subroundoff !< A thickness that is so small that
                             ...
                             !! changing it at the bit level [H ~> m or kg m-2].
      integer(c_int),    intent(in), value :: stencil !< The continuity solver stencil size with
      ...
      type(RealArray_C), intent(inout)     :: du_cor !< The zonal velocity increments from u that
                             !! give uhbt as the depth-integrated transports [L T-1 ~> m s-1].
      type(RealArray_C), intent(inout)     :: dv_cor !< The meridional velocity increments from v
                             !! that give vhbt as the depth-integrated transports
                             !! [L T-1 ~> m s-1].
    end subroutine turbotmp_continuity_ppm_bridge
  end interface

  interface
    !> Bridge for the continuity_2d_fluxes_PPM subroutine
    subroutine turbotmp_continuity_2d_fluxes_ppm_bridge(bx, u, v, h, uhbt, vhbt, dt, &
      ...
      type(RealArray_C), intent(in)        :: u   !< Zonal velocity [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: v   !< Meridional velocity [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: h   !< Layer thickness [H ~> m or kg m-2].
      type(RealArray_C), intent(inout)     :: uhbt !< Vertically summed thickness flux through
                                                    !! zonal faces [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C), intent(inout)     :: vhbt !< Vertically summed thickness flux through
                                                    !! meridional faces
                                                    !! [H L2 T-1 ~> m3 s-1 or kg s-1].
      real(c_double),    intent(in), value :: dt  !< Time increment [T ~> s].
      type(RealArray_C), intent(in)        :: mask2dT !< Cell land/ocean mask [nondim]
  ...
  interface
    !> Bridge for the continuity_3d_fluxes_PPM subroutine
    subroutine turbotmp_continuity_3d_fluxes_ppm_bridge(bx, u, v, h, uh, vh, dt, &
                                       mask2dT, dy_Cu, IareaT, IdxT, dxCu, areaT, dxT, &
                                       mask2dCu, dx_Cv, IdyT, dyCv, dyT, mask2dCv, &
                                       por_face_areaU, por_face_areaV, obc, &
      ...
      type(RealArray_C), intent(in)        :: v   !< Meridional velocity [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: h   !< Layer thickness [H ~> m or kg m-2].
      type(RealArray_C), intent(inout)     :: uh  !< Thickness fluxes through zonal faces,
                                                   !! u*h*dy [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C), intent(inout)     :: vh  !< Thickness fluxes through meridional faces,
                                                   !! v*h*dx [H L2 T-1 ~> m3 s-1 or kg s-1].
      real(c_double),    intent(in), value :: dt  !< Time increment [T ~> s].
      type(RealArray_C), intent(in)        :: mask2dT !< Cell land/ocean mask [nondim]
      type(RealArray_C), intent(in)        :: dy_Cu !< The grid cell's unblocked lengths of the
  ...
  interface
    !> Bridge for the continuity_adjust_vel_PPM subroutine
    subroutine turbotmp_continuity_adjust_vel_ppm_bridge(bx, u, v, h, uhbt, vhbt, dt, &
                                       mask2dT, dy_Cu, IareaT, IdxT, dxCu, areaT, dxT, &
                                       mask2dCu, dx_Cv, IdyT, dyCv, dyT, mask2dCv, &
                                       por_face_areaU, por_face_areaV, obc, &
                                       Angstrom_H, H_subroundoff, &
      ...
      type(RealArray_C), intent(in)        :: v   !< Meridional velocity [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: h   !< Layer thickness [H ~> m or kg m-2].
      type(RealArray_C), intent(in)        :: uhbt !< The vertically summed thickness flux
                             !! through zonal faces [H L2 T-1 ~> m3 s-1 or kg s-1].
      type(RealArray_C), intent(in)        :: vhbt !< The vertically summed thickness flux
                             !! through meridional faces [H L2 T-1 ~> m3 s-1 or kg s-1].
      real(c_double),    intent(in), value :: dt  !< Time increment [T ~> s].
      type(RealArray_C), intent(in)        :: mask2dT !< Cell land/ocean mask [nondim]
      type(RealArray_C), intent(in)        :: dy_Cu !< The grid cell's unblocked lengths of the
                                                     !! u/v-faces of the h-cell [L ~> m].
      ...
      type(RealArray_C), intent(inout)     :: v_cor !< The meridional velocities that give vhbt as
                                                     !! the depth-integrated transport
                                                     !! [L T-1 ~> m s-1].
      type(RealArray_C), intent(in)        :: visc_rem_u !< Both the fraction of the zonal
                             !! momentum that remains after a time-step of viscosity, and the
                             !! fraction of a time-step's worth of a barotropic acceleration that
                             !! a layer experiences after viscosity is applied [nondim].
      type(RealArray_C), intent(in)        :: visc_rem_v !< Both the fraction of the meridional
                             !! momentum that remains after a time-step of viscosity, and the
                             !! fraction of a time-step's worth of a barotropic acceleration that
                             !! a layer experiences after viscosity is applied [nondim].
    end subroutine turbotmp_continuity_adjust_vel_ppm_bridge
  end interface
...
public record_bt_cont, restore_bt_cont
...
type, public :: BT_cont_container_type ; private
  type(RealArray_t) :: FA_u_EE_a !< The effective open face area for zonal barotropic transport
                                 !! drawing from locations far to the east [H L ~> m2 or kg m-1].
  type(RealArray_t) :: FA_u_E0_a !< The effective open face area for zonal barotropic transport
                                 !! drawing from nearby to the east [H L ~> m2 or kg m-1].
  type(RealArray_t) :: FA_u_W0_a !< The effective open face area for zonal barotropic transport
                                 !! drawing from nearby to the west [H L ~> m2 or kg m-1].
  type(RealArray_t) :: FA_u_WW_a !< The effective open face area for zonal barotropic transport
                                 !! drawing from locations far to the west [H L ~> m2 or kg m-1].
  type(RealArray_t) :: uBT_WW_a  !< The barotropic velocity [L T-1 ~> m s-1], beyond which the
                                 !! marginal open face area is FA_u_WW_a. Must be non-negative.
  type(RealArray_t) :: uBT_EE_a  !< The barotropic velocity [L T-1 ~> m s-1], beyond which the
                                 !! marginal open face area is FA_u_EE_a. Must be non-positive.
  type(RealArray_t) :: FA_v_NN_a !< The effective open face area for meridional barotropic
                                 !! transport drawing from locations far to the north
                                 !! [H L ~> m2 or kg m-1].
  type(RealArray_t) :: FA_v_N0_a !< The effective open face area for meridional barotropic
                                 !! transport drawing from nearby to the north
                                 !! [H L ~> m2 or kg m-1].
  type(RealArray_t) :: FA_v_S0_a !< The effective open face area for meridional barotropic
                                 !! transport drawing from nearby to the south
                                 !! [H L ~> m2 or kg m-1].
  type(RealArray_t) :: FA_v_SS_a !< The effective open face area for meridional barotropic
                                 !! transport drawing from locations far to the south
                                 !! [H L ~> m2 or kg m-1].
  type(RealArray_t) :: vBT_SS_a  !< The barotropic velocity [L T-1 ~> m s-1], beyond which the
                                 !! marginal open face area is FA_v_SS_a. Must be non-negative.
  type(RealArray_t) :: vBT_NN_a  !< The barotropic velocity [L T-1 ~> m s-1], beyond which the
                                 !! marginal open face area is FA_v_NN_a. Must be non-positive.
  type(RealArray_t) :: h_u_a !< An effective thickness at zonal faces, taking into account the
                             !! effects of vertical viscosity and fractional open areas
                             !! [H ~> m or kg m-2]. Unassociated if the real BT_cont_type's h_u
                             !! was never allocated.
  type(RealArray_t) :: h_v_a !< An effective thickness at meridional faces, taking into account
                             !! the effects of vertical viscosity and fractional open areas
...
!> Returns true if this container was built from an associated BT_cont_type (i.e. the caller of
!! continuity() supplied a present, associated BT_cont). The 12 FA_*/uBT_*/vBT_* fields are always
!! allocated together (see alloc_BT_cont_type), so any one of them is a valid presence signal.
pure function BT_cont_container_associated(this) result(is_associated)
  class(BT_cont_container_type), intent(in) :: this !< The container to query
  logical :: is_associated !< True if this container was built from a real BT_cont_type

  is_associated = this%FA_u_EE_a%associated()
end function BT_cont_container_associated

!> Writes a BT_cont_container_type to a capture file by delegating to each field's own
!! %write_binary() in a fixed order -- unconditionally available under either infra layer,
!! unlike %to_c()/BT_cont_container_to_c, since it never touches the AMReX shadow buffer.
subroutine BT_cont_container_write_binary(this, unit)
  class(BT_cont_container_type), intent(in) :: this !< The container to write
  integer,                       intent(in) :: unit !< The file unit

  call this%FA_u_EE_a%write_binary(unit)
  call this%FA_u_E0_a%write_binary(unit)
  call this%FA_u_W0_a%write_binary(unit)
  call this%FA_u_WW_a%write_binary(unit)
  call this%uBT_WW_a%write_binary(unit)
  call this%uBT_EE_a%write_binary(unit)
  call this%FA_v_NN_a%write_binary(unit)
  call this%FA_v_N0_a%write_binary(unit)
  call this%FA_v_S0_a%write_binary(unit)
  call this%FA_v_SS_a%write_binary(unit)
  call this%vBT_SS_a%write_binary(unit)
  call this%vBT_NN_a%write_binary(unit)
  call this%h_u_a%write_binary(unit)
  call this%h_v_a%write_binary(unit)
end subroutine BT_cont_container_write_binary

!> Reads a BT_cont_container_type from a capture file, in the same fixed field order used by
!! BT_cont_container_write_binary.
subroutine BT_cont_container_read_binary(this, unit)
  class(BT_cont_container_type), intent(inout) :: this !< The container to read into
  integer,                       intent(in)    :: unit !< The file unit

  call this%FA_u_EE_a%read_binary(unit)
  call this%FA_u_E0_a%read_binary(unit)
  call this%FA_u_W0_a%read_binary(unit)
  call this%FA_u_WW_a%read_binary(unit)
  call this%uBT_WW_a%read_binary(unit)
  call this%uBT_EE_a%read_binary(unit)
  call this%FA_v_NN_a%read_binary(unit)
  call this%FA_v_N0_a%read_binary(unit)
  call this%FA_v_S0_a%read_binary(unit)
  call this%FA_v_SS_a%read_binary(unit)
  call this%vBT_SS_a%read_binary(unit)
  call this%vBT_NN_a%read_binary(unit)
  call this%h_u_a%read_binary(unit)
  call this%h_v_a%read_binary(unit)
end subroutine BT_cont_container_read_binary

!> Records a BT_cont_container_type into a capture file. io_recorder (turbotmp_helperF) cannot
!! gain its own add_bt_cont_container overload, since it would require using
!! MOM_continuity_PPM -- which already uses turbotmp_helperF, a circular module dependency. This
!! composes io_recorder's own already-public add_entry/unit_bin instead, mirroring exactly what
!! add_realarray does internally.
subroutine record_bt_cont(rec, name, BT_cont_a)
  type(io_recorder),            intent(inout) :: rec       !< The state recorder class
  character(*),                 intent(in)    :: name      !< The name of the variable
  type(BT_cont_container_type), intent(in)    :: BT_cont_a !< The container to write

  integer(kind=int64) :: pos

  inquire(unit=rec%unit_bin, pos=pos)
  call rec%add_entry(name, 'BT_cont_container_type', pos)
  call BT_cont_a%write_binary(rec%unit_bin)
end subroutine record_bt_cont

!> Restores a BT_cont_container_type from a capture file written by record_bt_cont.
subroutine restore_bt_cont(rec, name, BT_cont_a)
  type(io_recorder),            intent(inout) :: rec       !< The state recorder class
  character(*),                 intent(in)    :: name      !< The name of the variable
  type(BT_cont_container_type), intent(inout) :: BT_cont_a !< The container to read into

  integer :: idx
  integer(kind=int64) :: pos

  idx = rec%find_entry(name)
  if (idx < 0) call MOM_error(FATAL, "restore_bt_cont: variable "//trim(name)//" not found")

  if (trim(rec%entries(idx)%type_name) /= 'BT_cont_container_type') &
    call MOM_error(FATAL, "restore_bt_cont: variable "//trim(name)//" type mismatch")
...
function BT_cont_container_to_c(BT_cont_a) result(cdesc)
  ...
  cdesc%FA_u_EE = BT_cont_a%FA_u_EE_a%to_c()
  cdesc%FA_u_E0 = BT_cont_a%FA_u_E0_a%to_c()
  cdesc%FA_u_W0 = BT_cont_a%FA_u_W0_a%to_c()
  cdesc%FA_u_WW = BT_cont_a%FA_u_WW_a%to_c()
  cdesc%uBT_WW  = BT_cont_a%uBT_WW_a%to_c()
  cdesc%uBT_EE  = BT_cont_a%uBT_EE_a%to_c()
  cdesc%FA_v_NN = BT_cont_a%FA_v_NN_a%to_c()
  cdesc%FA_v_N0 = BT_cont_a%FA_v_N0_a%to_c()
  cdesc%FA_v_S0 = BT_cont_a%FA_v_S0_a%to_c()
  cdesc%FA_v_SS = BT_cont_a%FA_v_SS_a%to_c()
  cdesc%vBT_SS  = BT_cont_a%vBT_SS_a%to_c()
  cdesc%vBT_NN  = BT_cont_a%vBT_NN_a%to_c()
  cdesc%h_u     = BT_cont_a%h_u_a%to_c()
  cdesc%h_v     = BT_cont_a%h_v_a%to_c()
...
subroutine continuity_PPM_fortran(bxC, u_a, v_a, hin_a, h_a, uh_a, vh_a, dt, &
  ...
  type(RealArray_t),       intent(inout) :: v_cor_a
                             ...
                             !! transport [L T-1 ~> m s-1].
  type(BT_cont_container_type), intent(inout) :: BT_cont_a !< A structure with elements that
                             !!  describe the effective open face areas as a function of barotropic
                             !!  flow. Absence (from continuity()'s caller) is signaled by
                             !!  %associated() reading false.
...
subroutine continuity_PPM(bxC, u_a, v_a, hin_a, h_a, uh_a, vh_a, dt, &
  ...
  type(RealArray_C) :: mask2dT_c, IareaT_c, dy_Cu_c, IdxT_c, dxCu_c, areaT_c, dxT_c
  type(RealArray_C) :: mask2dCu_c, dx_Cv_c, IdyT_c, dyCv_c, dyT_c, mask2dCv_c
  ...
  select case (mode)
    ...
    case (TIMH_capture)
      ...
      if (capture) then
        ...
        call rec%add("_first_direction",  first_direction)
      ...
      call continuity_PPM_fortran(bxC, u_a, v_a, hin_a, h_a, uh_a, vh_a, dt, &
                          mask2dT_a, IareaT_a, dy_Cu_a, IdxT_a, dxCu_a, areaT_a, dxT_a, &
                          mask2dCu_a, dx_Cv_a, IdyT_a, dyCv_a, dyT_a, mask2dCv_a, &
                          por_face_areaU_a, por_face_areaV_a, OBC, &
                          first_direction, Angstrom_H, H_subroundoff, stencil, &
      ...
      if (capture) then
        ...
        call record_bt_cont(rec, "_BT_cont_after", BT_cont_a)
    ...
    case (TIMH_runAMREX)
      ...
      bt_cont_c          = BT_cont_container_to_c(BT_cont_a)
      ...
      call turbotmp_continuity_ppm_bridge(bxC_c, u_c, v_c, hin_c, h_c, uh_c, vh_c, dt, &
               mask2dT_c, IareaT_c, dy_Cu_c, IdxT_c, dxCu_c, areaT_c, dxT_c, &
...
subroutine continuity_3d_fluxes_PPM_fortran(bxC, u_a, v_a, h_a, uh_a, vh_a, dt, &
  ...
  ! Never built (no fields allocated); %associated() reads false, signaling BT_cont absent.
  type(BT_cont_container_type) :: no_BT_cont_a
...
end subroutine continuity_3d_fluxes_PPM_fortran

!> Shim for continuity_3d_fluxes_PPM -- dispatches via CONTINUITY_3D_FLUXES_PPM_MODE env var.
subroutine continuity_3d_fluxes_PPM(bxC, u_a, v_a, h_a, uh_a, vh_a, dt, &
  ...
  type(RealArray_C) :: mask2dT_c, dy_Cu_c, IareaT_c, IdxT_c, dxCu_c, areaT_c, dxT_c
  type(RealArray_C) :: mask2dCu_c, dx_Cv_c, IdyT_c, dyCv_c, dyT_c, mask2dCv_c
  type(RealArray_C) :: por_face_areaU_c, por_face_areaV_c
  type(Box_C)        :: bxC_c
  type(c_ptr)        :: OBC_c
  ...
  kernel = "continuity_3d_fluxes_ppm"

  mode = getenv_mode("CONTINUITY_3D_FLUXES_PPM_MODE", default=TIMH_runFORTRAN)
  ...
  select case (mode)
    ...
    case (TIMH_runAMREX)
      ...
      call turbotmp_continuity_3d_fluxes_ppm_bridge(bxC_c, u_c, v_c, h_c, uh_c, vh_c, dt, &
               mask2dT_c, dy_Cu_c, IareaT_c, IdxT_c, dxCu_c, areaT_c, dxT_c, &
               mask2dCu_c, dx_Cv_c, IdyT_c, dyCv_c, dyT_c, mask2dCv_c, &
               por_face_areaU_c, por_face_areaV_c, OBC_c, &
               Angstrom_H, H_subroundoff, &
...
end subroutine continuity_2d_fluxes_PPM_fortran

!> Shim for continuity_2d_fluxes_PPM -- dispatches via CONTINUITY_2D_FLUXES_PPM_MODE env var.
subroutine continuity_2d_fluxes_PPM(bxC, u_a, v_a, h_a, uhbt_a, vhbt_a, dt, &
  ...
  type(RealArray_C) :: mask2dT_c, dy_Cu_c, IareaT_c, IdxT_c, dx_Cv_c, IdyT_c
  type(RealArray_C) :: por_face_areaU_c, por_face_areaV_c
  type(Box_C)        :: bxC_c
  type(c_ptr)        :: OBC_c
  type(reconstruction_opts_C) :: recon_opts_c
  ...
  kernel = "continuity_2d_fluxes_ppm"

  mode = getenv_mode("CONTINUITY_2D_FLUXES_PPM_MODE", default=TIMH_runFORTRAN)
...
subroutine continuity_adjust_vel_PPM_fortran(bxC, u_a, v_a, h_a, uhbt_a, vhbt_a, dt, &
...
end subroutine continuity_adjust_vel_PPM_fortran

!> Shim for continuity_adjust_vel_PPM -- dispatches via CONTINUITY_ADJUST_VEL_PPM_MODE env var.
subroutine continuity_adjust_vel_PPM(bxC, u_a, v_a, h_a, uhbt_a, vhbt_a, dt, &
  ...
  kernel = "continuity_adjust_vel_ppm"

  mode = getenv_mode("CONTINUITY_ADJUST_VEL_PPM_MODE", default=TIMH_runFORTRAN)
  ...
  select case (mode)
    ...
    case (TIMH_capture)
      ...
      call continuity_adjust_vel_PPM_fortran(bxC, u_a, v_a, h_a, uhbt_a, vhbt_a, dt, &
                                     mask2dT_a, dy_Cu_a, IareaT_a, IdxT_a, dxCu_a, areaT_a, &
                                     dxT_a, mask2dCu_a, dx_Cv_a, IdyT_a, dyCv_a, dyT_a, &
                                     mask2dCv_a, por_face_areaU_a, por_face_areaV_a, OBC, &
                                     Angstrom_H, H_subroundoff, &
    ...
    case (TIMH_runAMREX)
      ...
      call turbotmp_continuity_adjust_vel_ppm_bridge(bxC_c, u_c, v_c, h_c, uhbt_c, vhbt_c, dt, &
               mask2dT_c, dy_Cu_c, IareaT_c, IdxT_c, dxCu_c, areaT_c, dxT_c, &
               mask2dCu_c, dx_Cv_c, IdyT_c, dyCv_c, dyT_c, mask2dCv_c, &
               por_face_areaU_c, por_face_areaV_c, OBC_c, &
               Angstrom_H, H_subroundoff, &
...
subroutine continuity_zonal_convergence_fortran(bxC, h_a, uh_a, dt, IareaT_a, hin_a, hmin)
  ...
  real,                                       intent(in)    :: hmin !< The minimum layer
  ...
  if (hin_a%associated()) then
    ...
    do concurrent(k=bxC%idxS(3):bxC%idxE(3), &
      ...
      h(i,j,k) = max( hin(i,j,k) - dt * IareaT(i,j) * (uh(I,j,k) - uh(I-1,j,k)), hmin )
    enddo
  else
    ! untested
    do concurrent(k=bxC%idxS(3):bxC%idxE(3), &
                  j=bxC%idxS(2):bxC%idxE(2), &
                  i=bxC%idxS(1):bxC%idxE(1))
      h(i,j,k) = max( h(i,j,k) - dt * IareaT(i,j) * (uh(I,j,k) - uh(I-1,j,k)), hmin )
    enddo
  endif

end subroutine continuity_zonal_convergence_fortran

!> Shim for continuity_zonal_convergence -- dispatches via CONTINUITY_ZONAL_CONVERGENCE_MODE
!! env var.
...
subroutine continuity_meridional_convergence_fortran(bxC, h_a, vh_a, dt, IareaT_a, hin_a, hmin)
  ...
  if (hin_a%associated()) then
    ...
    do concurrent(k=bxC%idxS(3):bxC%idxE(3), &
                  j=bxC%idxS(2):bxC%idxE(2), &
                  i=bxC%idxS(1):bxC%idxE(1))
      h(i,j,k) = max( hin(i,j,k) - dt * IareaT(i,j) * (vh(i,J,k) - vh(i,J-1,k)), hmin )
    enddo
  else
    do concurrent(k=bxC%idxS(3):bxC%idxE(3), &
                  j=bxC%idxS(2):bxC%idxE(2), &
                  i=bxC%idxS(1):bxC%idxE(1))
      h(i,j,k) = max( h(i,j,k) - dt * IareaT(i,j) * (vh(i,J,k) - vh(i,J-1,k)), hmin )
    enddo
  endif

end subroutine continuity_meridional_convergence_fortran

!> Shim for continuity_meridional_convergence -- dispatches via
!! CONTINUITY_MERIDIONAL_CONVERGENCE_MODE env var.
subroutine continuity_meridional_convergence(bxC, h_a, vh_a, dt, IareaT_a, hin_a, hmin)
  ...
  select case (mode)
    ...
    case (TIMH_runAMREX)
      ...
      hin_c    = hin_a%to_c()
      hmin_c   = hmin
      call turbotmp_continuity_meridional_convergence_bridge(bxC_c, h_c, vh_c, dt, IareaT_c, &
               hin_c, hmin_c)
#endif
...
subroutine zonal_mass_flux_fortran(bxC, u_a, h_in_a, h_W_a, h_E_a, uh_a, dt, OBC, &
  ...
  type(BT_cont_container_type), intent(inout) :: BT_cont_a !< A structure with elements that
                     ...
                     !! Absence is signaled by %associated() reading false.
  ...
  set_BT_cont = BT_cont_a%associated()
...
end subroutine zonal_mass_flux_fortran

!> Shim for zonal_mass_flux -- dispatches via ZONAL_MASS_FLUX_MODE env var.
subroutine zonal_mass_flux(bxC, u_a, h_in_a, h_W_a, h_E_a, uh_a, dt, OBC, &
  ...
  type(RealArray_t),       intent(inout) :: u_cor_a
                     !< The zonal velocities (u with a barotropic correction)
                     !! that give uhbt as the depth-integrated transport [L T-1 ~> m s-1]
  type(BT_cont_container_type), intent(inout), target :: BT_cont_a !< A structure with elements
                     !! that describe the effective open face areas as a function of barotropic
                     !! flow. Absence is signaled by %associated() reading false. The `target`
                     !! attribute is needed so the AMReX arm can take its address via c_loc().
  type(RealArray_t),       intent(inout) :: du_cor_a
                                     !< The zonal velocity increments from u that give uhbt
                                     !! as the depth-integrated transports [L T-1 ~> m s-1].
  type(RealArray_t),       intent(in)    :: dy_Cu_a !< The grid cell's unblocked lengths of the
  ...
  type(RealArray_C) :: uhbt_c, visc_rem_u_c, u_cor_c, du_cor_c
  ...
  kernel = "zonal_mass_flux"

  call cpu_clock_begin(id_clock_correct)

  mode = getenv_mode("ZONAL_MASS_FLUX_MODE", default=TIMH_runFORTRAN)
  ...
  select case (mode)
    ...
    case (TIMH_runAMREX)
      ...
      if (BT_cont_a%associated()) then
        BT_cont_c = c_loc(BT_cont_a)
      else
        BT_cont_c = c_null_ptr
      endif
...
subroutine present_uhbt_or_set_BT_cont_fortran(bxC, u_a, h_in_a, h_W_a, h_E_a, uh_tot_0_a, &
  ...
  if (set_BT_cont) then
    call BT_cont_a%FA_u_W0_a%view(FA_u_W0)
    call BT_cont_a%FA_u_E0_a%view(FA_u_E0)
    call BT_cont_a%FA_u_WW_a%view(FA_u_WW)
    call BT_cont_a%FA_u_EE_a%view(FA_u_EE)
    call BT_cont_a%uBT_WW_a%view(uBT_WW)
    call BT_cont_a%uBT_EE_a%view(uBT_EE)
  endif
...
end subroutine present_uhbt_or_set_BT_cont_fortran

!> Shim for present_uhbt_or_set_BT_cont -- dispatches via PRESENT_UHBT_OR_SET_BT_CONT_MODE env var.
subroutine present_uhbt_or_set_BT_cont(bxC, u_a, h_in_a, h_W_a, h_E_a, uh_tot_0_a, &
  ...
  type(RealArray_C) :: du_max_CFL_c, du_min_CFL_c, visc_rem_u_c, visc_rem_max_c, por_face_areaU_c
  type(RealArray_C) :: uhbt_c, uh_c, u_cor_c, du_cor_c, dy_Cu_c, IareaT_c, IdxT_c, dxCu_c
  type(BT_cont_C)    :: bt_cont_c
  type(Box_C)        :: bxC_c
  type(c_ptr)        :: OBC_c
  type(transport_adjust_opts_C) :: adjust_opts_c
  ...
  kernel = "present_uhbt_or_set_bt_cont"

  mode = getenv_mode("PRESENT_UHBT_OR_SET_BT_CONT_MODE", default=TIMH_runFORTRAN)
  ...
  select case (mode)
    ...
    case (TIMH_capture)
      ...
      call present_uhbt_or_set_BT_cont_fortran(bxC, u_a, h_in_a, h_W_a, h_E_a, uh_tot_0_a, &
                                       duhdu_tot_0_a, du_a, du_max_CFL_a, du_min_CFL_a, &
                                       visc_rem_u_a, visc_rem_max_a, por_face_areaU_a, uhbt_a, &
                                       uh_a, u_cor_a, du_cor_a, BT_cont_a, dt, OBC, &
                                       dy_Cu_a, H_subroundoff, IareaT_a, IdxT_a, dxCu_a, &
...
subroutine zonal_BT_mass_flux(bxC, u_a, h_in_a, h_W_a, h_E_a, uhbt_a, dt, adjust_opts, &
  ...
  integer  :: mode, rc
  type(RealArray_C) :: u_c, h_in_c, h_W_c, h_E_c, uhbt_c, por_face_areaU_c
...
subroutine zonal_flux_adjust(bxC, u_a, h_in_a, h_W_a, h_E_a, uh_tot_0_a, duhdu_tot_0_a, &
  ...
  integer  :: mode, rc
  type(RealArray_C) :: u_c, h_in_c, h_W_c, h_E_c, visc_rem_c, uhbt_c, du_max_CFL_c, du_min_CFL_c
  type(RealArray_C) :: uh_tot_0_c, duhdu_tot_0_c, du_c, por_face_areaU_c, uh_3d_c
  type(RealArray_C) :: dy_Cu_c, IareaT_c, IdxT_c
  type(LogicalArray_C) :: do_I_in_c
  type(LogicalArray_t) :: do_I_in_local ! Shallow copy so %to_c() can be called on an intent(in)
...
subroutine set_zonal_BT_cont_fortran(bxC, u_a, h_in_a, h_W_a, h_E_a, BT_cont_a, du0_a, &
  ...
  type(RealArray_t),       intent(in)    :: h_E_a !< East edge thickness in the reconstruction
                                                  !! [H ~> m or kg m-2].
  type(BT_cont_container_type), intent(inout) :: BT_cont_a !< A structure with elements
                       !! that describe the effective open face areas as a function of barotropic
                       !! flow.
...
subroutine set_zonal_BT_cont(bxC, u_a, h_in_a, h_W_a, h_E_a, BT_cont_a, du0_a, &
  ...
  integer  :: mode, rc
  type(RealArray_C) :: u_c, h_in_c, h_W_c, h_E_c, du0_c, uh_tot_0_c, duhdu_tot_0_c
  type(RealArray_C) :: du_max_CFL_c, du_min_CFL_c, visc_rem_c, visc_rem_max_c, por_face_areaU_c
  type(RealArray_C) :: dxCu_c, dy_Cu_c, IareaT_c, IdxT_c
  type(BT_cont_C)    :: bt_cont_c
  type(LogicalArray_C) :: do_I_c
  type(LogicalArray_t) :: do_I_local ! Shallow copy so %to_c() can be called on an intent(in)
                                      ! dummy; %free_c() below frees only its own shadow buffer,
...
end subroutine meridional_mass_flux_fortran

!> Shim for meridional_mass_flux -- dispatches via MERIDIONAL_MASS_FLUX_MODE env var.
subroutine meridional_mass_flux(bxC, v_a, h_in_a, h_S_a, h_N_a, vh_a, dt, OBC, &
  ...
  type(RealArray_C) :: vhbt_c, visc_rem_v_c, v_cor_c, dv_cor_c
  ...
  kernel = "meridional_mass_flux"

  call cpu_clock_begin(id_clock_correct)

  mode = getenv_mode("MERIDIONAL_MASS_FLUX_MODE", default=TIMH_runFORTRAN)
...
subroutine present_vhbt_or_set_BT_cont_fortran(bxC, v_a, h_in_a, h_S_a, h_N_a, vh_tot_0_a, &
  ...
  if (set_BT_cont) then
    call BT_cont_a%FA_v_S0_a%view(FA_v_S0)
    call BT_cont_a%FA_v_N0_a%view(FA_v_N0)
    call BT_cont_a%FA_v_SS_a%view(FA_v_SS)
    call BT_cont_a%FA_v_NN_a%view(FA_v_NN)
    call BT_cont_a%vBT_SS_a%view(vBT_SS)
    call BT_cont_a%vBT_NN_a%view(vBT_NN)
  endif
...
end subroutine present_vhbt_or_set_BT_cont_fortran

!> Shim for present_vhbt_or_set_BT_cont -- dispatches via PRESENT_VHBT_OR_SET_BT_CONT_MODE env var.
subroutine present_vhbt_or_set_BT_cont(bxC, v_a, h_in_a, h_S_a, h_N_a, vh_tot_0_a, &
  ...
  type(RealArray_C) :: dv_max_CFL_c, dv_min_CFL_c, visc_rem_v_c, visc_rem_max_c, por_face_areaV_c
  type(RealArray_C) :: vhbt_c, vh_c, v_cor_c, dv_cor_c, dx_Cv_c, IareaT_c, IdyT_c, dyCv_c
  type(BT_cont_C)    :: bt_cont_c
  type(Box_C)        :: bxC_c
  type(c_ptr)        :: OBC_c
  type(transport_adjust_opts_C) :: adjust_opts_c
  ...
  kernel = "present_vhbt_or_set_bt_cont"

  mode = getenv_mode("PRESENT_VHBT_OR_SET_BT_CONT_MODE", default=TIMH_runFORTRAN)
  ...
  select case (mode)
    ...
    case (TIMH_capture)
      ...
      call present_vhbt_or_set_BT_cont_fortran(bxC, v_a, h_in_a, h_S_a, h_N_a, vh_tot_0_a, &
                                       dvhdv_tot_0_a, dv_a, dv_max_CFL_a, dv_min_CFL_a, &
                                       visc_rem_v_a, visc_rem_max_a, por_face_areaV_a, vhbt_a, &
                                       vh_a, v_cor_a, dv_cor_a, BT_cont_a, dt, OBC, &
                                       dx_Cv_a, H_subroundoff, IareaT_a, IdyT_a, dyCv_a, &
...
subroutine meridional_BT_mass_flux(bxC, v_a, h_in_a, h_S_a, h_N_a, vhbt_a, dt, &
  ...
  integer  :: mode, rc
  type(RealArray_C) :: v_c, h_in_c, h_S_c, h_N_c, vhbt_c, por_face_areaV_c
...
subroutine meridional_flux_adjust(bxC, v_a, h_in_a, h_S_a, h_N_a, vh_tot_0_a, &
  ...
  integer  :: mode, rc
  type(RealArray_C) :: v_c, h_in_c, h_S_c, h_N_c, visc_rem_c, vhbt_c, dv_max_CFL_c, dv_min_CFL_c
  type(RealArray_C) :: vh_tot_0_c, dvhdv_tot_0_c, dv_c, por_face_areaV_c, vh_3d_c
  type(RealArray_C) :: dx_Cv_c, IareaT_c, IdyT_c
  type(LogicalArray_C) :: do_I_in_c
  type(LogicalArray_t) :: do_I_in_local ! Shallow copy so %to_c() can be called on an intent(in)
                                      ! dummy; %free_c() below frees only its own shadow buffer,
...
subroutine set_merid_BT_cont_fortran(bxC, v_a, h_in_a, h_S_a, h_N_a, BT_cont_a, dv0_a, &
  ...
  type(RealArray_t),       intent(in)    :: h_N_a !< North edge thickness in the reconstruction,
                                                  !! [H ~> m or kg m-2].
  type(BT_cont_container_type),                intent(inout) :: BT_cont_a !< A structure with
                       !! elements that describe the effective open face areas as a function of
                       !! barotropic flow.
...
subroutine set_merid_BT_cont(bxC, v_a, h_in_a, h_S_a, h_N_a, BT_cont_a, dv0_a, &
  ...
  integer  :: mode, rc
  type(RealArray_C) :: v_c, h_in_c, h_S_c, h_N_c, dv0_c, vh_tot_0_c, dvhdv_tot_0_c
  type(RealArray_C) :: dv_max_CFL_c, dv_min_CFL_c, visc_rem_c, visc_rem_max_c, por_face_areaV_c
  type(RealArray_C) :: dyCv_c, dx_Cv_c, IareaT_c, IdyT_c
  type(BT_cont_C)    :: bt_cont_c
  type(LogicalArray_C) :: do_I_c
  type(LogicalArray_t) :: do_I_local ! Shallow copy so %to_c() can be called on an intent(in)
                                      ! dummy; %free_c() below frees only its own shadow buffer,
